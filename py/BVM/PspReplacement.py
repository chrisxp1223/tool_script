...
#==========required only if sign enabled start==========#
  #Sign BIOS need to add entry 0x5 in each BIOS directory(if entry 0x5 already exists, modify instead).
  #The token for this entry can be found at: https://confluence.amd.com/display/MMDS/AMD+Reference+Platform+BIOS+Signing+Key+Tokens
  {
  "entryType" : "IMAGE_ENTRY",
  "type" : "0x5",
  "romId" : "0x0",
  ...
  "subProgram" : "0x0",
  "operation" : EntryOperationType.Modify.value,
  "filename" : "C:\\Users\\lahan\\Desktop\\BVM\\Rembrandt-4K-BIOS-SBR-0110.stkn",
  "level" : "0x2A",
  "dirIndex" : "0x0",
  "isPspEntry" : False
  },
  ...
  #Sign BIOS also need to add entry 0x7 in each BIOS directory(if entry 0x7 already exists, modify instead).
  #Leave filename blank, this entry does not require upload anything
  #Size is required for this entry: 2K key => 0x100, 4K key => 0x200
  {
  "entryType" : "IMAGE_ENTRY",
  "type" : "0x7",
  "romId" : "0x0",
...
def SubmitPspRequest(requestId, signType, signHP, signUserName, signPassword, replacementList, token):
  ...
  url = url + "&signSpFunction=" + signSpFunction
  headers = {"Authorization": "Bearer " + token, "Content-Type" : "application/json"}
  
  #Update filename to BVM_apifile.bin
  for r in replacementList:
    r["filename"] = "BVM_apifile.bin"
    #Override for entry 0x7. The name depends on the PSP structure. Possible names are: RTMSignature, RTMSignatureL1L2, RTMSignatureL1L2B
    if(r["type"] == "0x7"):
      r["filename"] = "RTMSignatureL1L2.bin"
