...
processorName = "Krackan - Family 1Ah"
platformName = "Rev_KRK_Aransas_AMD_EDKII"
baseBIOSType = "User-Generated"
revision = "D:\\Tools\\BVMAPI\\STXKRK_KORATPLUS_DDR5_INT_4202.FD"
cbsConfigProgram = "Rev_KRK_Aransas_AMD_EDKII"
...
#0 means CBS Override function, 1 means GBS modification, 2 means Binary Level CBS Override. GBS modification is not supported by API
isGbs = "2"
purpose = "BVMAPI Binary CBS Override test"
...
def UploadUserGenBIOS(revision, token):
  ...
  if(response.status_code == 200):
    return response.content.decode("ascii").replace("\"","").replace("\\\\","\\")
  else:
...
def SubmitCbsRequest(requestId, replacementList, token):
  log.info("Submitting Binary CBS request......")
  url = baseUrl + "api/bvmcbsoverride/SubmitBinaryLevelCbsOverrideRequest"
  headers = {"Authorization": "Bearer " + token}
  argument_dict = dict()
  ...
  if(response.status_code != 200):
      ...
      return None
  log.info("Submit Binary CBS Request successful")
  return 1
